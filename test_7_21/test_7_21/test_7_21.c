...
//		for (j = 0; j < sz - 1 - i; j++)
//		{
//			if (cmp((char*)base + j * width, (char*)base + (j + 1) * width) > 0)//待比较的两个元素的位置，This is the most important point
//			{
...
//	int a[3][4] = { 0 };
//	printf("%d\n", sizeof(a[0] + 1));
//	//a[0]并没有放在sizeof内部，也没取地址，a[0]就表示首元素的地址
//	printf("%d\n", sizeof(a + 1));
//	/*a虽然表示二维数组的地址，但是并没有放在sizeof内部也没取地址
//	所以a表示首元素的地址，二维数组首元素是它的第一行，a就是第一行的地址
//	a+1表示跳过第一行，指向第二行*/
//	printf("%d\n", sizeof(*(a + 1)));
//	//*(a+1)-->a[1]
//	printf("%d\n", sizeof(a[3]));//不会越界
...
//	char*** cpp = cp;
//
//	printf("%s\n", **++cpp);//这个++cpp会改变cpp的值，从而后面的cpp都要从新的位置开始考虑
//	printf("%s\n", *-- * ++cpp + 3);//++,--运算符会改变cpp的值，使得它不再是原来的值
//	printf("%s\n", *cpp[-2]+3);
//	printf("%s\n", cpp[-1][-1]+1);
//
//	return 0;
